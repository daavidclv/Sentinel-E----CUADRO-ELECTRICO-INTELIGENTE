...
{
  /* Suspender Systick para que no despierte al micro cada 1ms */
  HAL_SuspendTick();

  /* Apagamos el LED verde/rojo que indique actividad para ahorrar batería */
  LED_Apagado(0);
  LED_Apagado(2);

  /* Entrar en modo STOP (Regulador en Low Power) */
  HAL_PWR_EnterSTOPMode(PWR_LOWPOWERREGULATOR_ON, PWR_STOPENTRY_WFI);

  /* --- EL MICRO SE QUEDA AQUÍ DORMIDO HASTA LA ALARMA DEL RTC --- */

  /* Reconfigurar el reloj del sistema tras despertar de STOP */
  SYSCLKConfig_STOP();
...
void Configurar_WakeUp_RTC_55s(void) {
     ...
     * Usamos RTC_WAKEUPCLOCK_CK_SPRE_16BITS que equivale a 1 Hz (1 tick = 1 segundo).
     * Por lo tanto, 55 ticks = 55 segundos exactos en modo STOP. 
     */
